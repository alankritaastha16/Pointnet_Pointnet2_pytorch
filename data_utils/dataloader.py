...
WARNING: the repository we are using for PointNet++ uses a different format
(N, K). We have updated some dataloaders (like ShapeNet), but not all.
'''
...
class ICCV17ShapeNet(Dataset):
    ...
    def __init__(self, root, fold, transform, segmentation='part', npoints=2500):
        assert fold in ('train', 'val', 'test')
        assert segmentation in ('part', 'seg', 'cls')
        ...
        self.n_cls_classes = len(self.labels)
        self.n_seg_classes = 6
        self.classes = []
        self.objfnames = []
        self.segfnames = []
    ...
    def __getitem__(self, i):
        P = np.loadtxt(self.objfnames[i]).astype(np.float32)
        Y = self.classes[i]
        S = np.loadtxt(self.segfnames[i]).astype(np.int64)-1

        # resample
        choice = np.random.choice(len(S), self.npoints, replace=True)
        P = P[choice, :]
        S = S[choice]

        if self.transform:
            P, S = self.transform(P, S)
        if self.segmentation == 'seg': return P, S
        if self.segmentation == 'cls': return P, Y
        if self.segmentation == 'part': return P, S, Y

class ICCV17ShapeNetClass(ICCV17ShapeNet):
    '''Convenience wrapper for `ICCV17ShapeNet` with `segmentation=False`.'''
    def __init__(self, root, fold, transform):
        super().__init__(root, fold, transform, 'cls')

class ICCV17ShapeNetSeg(ICCV17ShapeNet):
    '''Convenience wrapper for `ICCV17ShapeNet` with `segmentation=True`.'''
    def __init__(self, root, fold, transform):
        super().__init__(root, fold, transform, 'seg')
