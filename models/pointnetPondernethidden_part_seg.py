...
class get_model(nn.Module):
    def __init__(self, part_num=50, extra_input_size=0,normal_channel=True):
        ...
        self.stn = STN3d(channel+ extra_input_size)
        self.conv1 = torch.nn.Conv1d(channel+ extra_input_size, 64, 1)
    ...
    def forward(self, point_cloud, label):
        ...
        net1 = F.relu(self.bns2(self.convs2(net)))
        net = F.relu(self.bns3(self.convs3(net1)))
        net = self.convs4(net)
        net = net.transpose(2, 1).contiguous()
        net = F.log_softmax(net.view(-1, self.part_num), dim=-1)
        net = net.view(B, N, self.part_num) # [B, N, 50]

        return net, trans_feat,net1
