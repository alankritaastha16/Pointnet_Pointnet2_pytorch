...
def main(args):
    ...
    with torch.no_grad():
        ...
        classifiers = [classifiers[i].eval() for i in range(num_itr)]
        for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader),
                                                      smoothing=0.9):
            #batchsize, num_point, _ = points.size()
            cur_batch_size, NUM_POINT, _ = points.size()
            prev_output = torch.zeros((points.shape[0], input_size, points.shape[1]), device=device)
            ...
            for j in range(num_itr):
                ...
                seg_pred, _ ,h1_layer= classifiers[j](_points, to_categorical(label, num_classes))
