...
def parse_args():
    ...
    parser.add_argument('--num_itr', default=5, type=int, help='number of iterations for inference')
   # parser.add_argument('--subset', default=13998, type=int, help='no of samples in dataset [default: 10000]')
...
def main(args):
   ...
   # TRAIN_DATASET = torch.utils.data.Subset(TRAIN_DATASET, range(0, 2500))
   # print('after sub-sample:', len(TRAIN_DATASET))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total = len(trainDataLoader), smoothing=0.9):
            prev_output = torch.zeros((points.shape[0], 128, points.shape[1]), device=device)
            ...
            for j in range(num_itr):
                ...
                seg_pred, trans_feat, h1_layer = classifiers[j](_points, to_categorical(label, num_classes))
                #print(h1_layer.shape)
                prev_output = h1_layer.detach()
